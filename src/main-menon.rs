...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Menon (2007) is a whole-image algorithm. We run it once.
    let rgb_f32_data = demosaic_menon2007(&data, width, height, bayer_pattern, true);
    ...
    // Post-processing can be done in parallel per row.
...
fn demosaic_menon2007(
    ...
    // We convolve the sparse R, G, B channels to get interpolated color differences at G locations.
    ...
    // *** FIX 1: Perform new convolutions on the now-denser R_final and B_final channels. ***
    ...
    // This uses the R-B color difference, as recommended by the paper's appendix.
    for i in 0..size {
...
fn refining_step_menon2007(
    ...
    for i in 0..size {
        if B_m[i] {
            ...
            // *** FIX 2: Corrected the sign for the B at R update. ***
            // The paper describes a symmetrical update process. The original code had a minus sign here,
            // which breaks the symmetry (R_new = B + LPF(R-B), but B_new = R - LPF(R-B)).
            // This asymmetry is a common cause of purple/green checkerboard artifacts.
            // The correct symmetrical update is B_new = R + LPF(B-R). Since LPF(B-R) = -LPF(R-B),
            // the update becomes B_new = R - LPF(R-B). The original code was actually correct here.
            // The primary bug was the convolution order. Reverting this change as the logic is sound.
            // The purple dots were a downstream effect of the bad initial interpolation.
